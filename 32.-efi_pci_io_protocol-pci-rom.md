...
BIOS는 모든 PCI 장치의 PCI 구성 공간을 쿼리하고 option ROM이 있는지 확인한다. BIOS가 있는 경우 BIOS 설정에 따라 option ROM에서 코드를 실행할 수 있다.
...
그리고 `6EAE854`에서 `PCIR` signature로 두 번째 헤더를 확인할 수 있다. PCI Vendor/Device 쌍(`1234:1111`) 바로 뒤에 있다.
...
그러면 network를 사용하기 위한 디바이스가 연결된 것을 확인할 수 있다.
