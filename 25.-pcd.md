...
&#x20;여기서 눈치가 빠른 사람은 느꼈겠지만, 어딘가 이상한 부분이 존재한다. 바로 '0'인데, LibPcdGet32의 매개변수 값은 LocalToken 번호이다. 그리고 앞선 Lesson에서 언급한 것처럼, LocalToken 번호는 1부터 시작한다.

&#x20;따라서 0은 유효하지 않은 토큰 번호이다. 여기서 Application을 build하고 실행하면 오류가 발생할 수 있다.



&#x20;LibPcdGet과 LibPcdSet API는 현재 PCD 데이터베이스에 접근하기 위한 것이다. 따라서 OVMF에서 application을 실행할 때, LibPcdGet, LibPcdSet은 OVMF PCD 데이터베이스에 접근한다.



&#x20;그러나, PCD 데이터베이스는 정적이므로 이미지를 build할 때, 모든 PCD를 수집해야 한다. Runtime 시 데이터베이스에 더 많은 Dynamic PCD를 추가할 수 없다. 어떤 Dynamic PCD가 PCD 데이터베이스로 이동하는 지를 살펴보면, 최종 Flash Image로 이동하는 모듈의 Dynamic PCD만 이동한다.



&#x20;일반적인 원리는 다음과 같다.

* 최종 Flash image로 이동하는 \[DynamicPcds]의 모든 모듈에 대해 LocalToken 번호를 할당한다. 1부터 시작하는 전역(global) Platform 레벨 할당이다.
* 위의 모든 \[DynamicPcds]을 사용하여 PCD\_IS\_DRIVER = PEI\_PCD\_DRIVER / PXE\_PCD\_DRIVER 모듈에 대한 PCD DB 이미지 생성



&#x20;우리의 application은 OVMF 이미지의 일부가 아니기 때문에, OVMF PCD 데이터베이스에 PcdDynamicInt32에 대해서는 아직 알지 못한다. 그리고 우리 패키지는 Flash image 자체를 생성하지 않는다. 따라서 build 시스템은 PCD에 Local Token 번호를 할당할 수 없다.



&#x20;따라서 Dynamic PCD가 있는 모듈이 PCD 데이터베이스가 있는 메인 Firmware와 별도로 컴파일되었기 때문에 오류가 발생했다. 이 문제를 해결하려면 Application을 OVMF Flash Image에 포함해야 한다.
...
&#x20;그리고 PCD DB parser 유틸리티를 실행하면, 최대 LocalToken의 개수가 하나 더 많아진 것을 확인할 수 있다. 매직 넘버 값으로 PCD를 찾을 수도 있다.
...
&#x20;이제 application AutoGen 파일을 확인해보겠다. 현재 우리 application의 OvmfX64 build 버전은 Build/UefiLessonsPkg/RELEASE\_GCC5/X64/UefiLessonsPkg/PCDLesson 폴더에 있는 것이 아니라, Build/OvmfX64/RELEASE\_GCC5/X64/UefiLessonsPkg/PCDLesson 폴더에 존재함을 기억하자.



&#x20;따라서 AutoGen.h를 보면, PCD에 올바른 Token 번호가 할당된 것을 볼 수 있다.
...
&#x20;이제 Get, Set method가 올바르게 작동한다.

&#x20;application을 한 번 더 실행하면, 마지막 프로그램에 대한 호출이 PCD 값을 변경함을 알 수 있다.
...
&#x20;그러나 이 내용은 해당 부팅 과정에만 해당한다. PCD 데이터베이스 모듈은 Flash에서 RAM으로 데이터베이스 파일을 로드하고 모든 PCD에 대한 변경 사항은 Flash code에 영향을 주지 않고 현재 RAM 값만 수정한다.
...
&#x20;항상 OvmfPkg의 일부로 application을 사용하는 것은 아니므로, PcdDynamicInt LocalToken이 할당되지 않은 경우 액세스를 보호하는 것이 좋다.
...
&#x20;PCD 데이터베이스에 필요한 PCD가 이미 존재한다고 가정해보자. 그리고 UEFI shell에서 application을 통해 접근하려고 한다.



&#x20;Dynamic PCD로 이를 수행하는 유일한 방법은 이 application을 이미지 build할 때 포함시키는 방법이다. 다른 경우에는 application을 별도로 컴파일할 때, build 시스템은 image PCD DB에서 사용되는 Local Token No. 지정 시스템을 인식하고 Token 번호로 0을 생성한다.



&#x20;그러나 때로는 Flash image에 대한 소스가 없다. 따라서 appplication을 이미지의 일부로 컴파일하는 것은 불가능하다. 이때 무엇을 할 수 있을까?
...
&#x20;모든 준비가 끝났으니, 다시 OVMF image를 build해보자.
...
&#x20;DynamicEx Token 값은 DEC 파일에 넣어준 코드의 Token 번호이다.
...
&#x20;중요한 것은 UefiLessonsPkg를 build할 경우,

```
$ build
```

&#x20;Application은 AutoGen.h 파일에서 동일한 코드를 가져온다.



&#x20;따라서 OVMF 소스 코드에 대한 연결이 없더라도 build 시스템은 필요한 PCD에 액세스하기 위해 올바른 credential을 생성한다. 우리는 패키지에 대한 다른 application을 만들 수 있으며, 여전히 앱으로 만든 것과 동일한 CPD에 액세스하는 데 사용할 수 있다.



&#x20;이것이 Dynamic 및 DynamicPCD의 주요한 차이점이다. 기본 Flash Image의 일부로 application을 build할 필요가 없다.
...
&#x20;이제 이 application을 build하고 결과를 공유디스크에 복사한다. 다시 말하지만, DynamicEx PCD의 경우 패키지 빌드 결과, 즉 Build/UefiLessonsPkg/RELEASE\_GCC5/X64/UefiLessonsPkg/PCDLesson/\
PCDLesson/DEBUG/PCDLesson.efi 파일을 사용할 수 있다.

&#x20;(Build가 이루어지지 않을 수 있는데, 코드에 대한 검토를 진행하고 수정하도록 하겠다.)
...
&#x20;PcdGet32 / PcdSet32S 함수가 특정 Dynamic API 함수가 아니라는 점에 주의해야 한다. 이러한 기능은 DynamicEx PCD를 포함한 모든 유형의 PCD와 함께 사용할 수 있는 일반 API이다. 예를 들어, Application에 다음 코드를 추가한다.
...
&#x20;Application을 다시 build하여 잘 작동하는 지 확인할 수 있다.
...
&#x20;EDK2에는 모든 Dynamic PCD를 Dynamic Ex 유형으로 쉽게 변경할 수 있는 방법이 있다. 이를 위해 Platform DSC 파일에 특수 정의를 추가하기만 하면 된다.
...
&#x20;OVMF를 다시 build할 때 위의 정의를 추가하면 많은 양의 re-compile이 발생한다.

&#x20;build한 이후 PCD DB 파일을 확인하자.
...
&#x20;OVMF가 부팅되면 application이 예상대로 작동하는 지 확인할 수 있다.
...
&#x20;또 다른 Platform level PCD 정의가 있다. PCD\_INFO\_GENERATION인데, 이 설정을 DSC 파일에 추가한다. (그리고 PCD\_DYNAMIC\_AS\_DYNAMICEX = TRUE 제거)
...
&#x20;그 다음, OVMF를 다시 컴파일하고 PCD DB 파일을 확인한다.
...
&#x20;이제 모든 Token에는 TokenSpaceName / PcdName Field가 존재한다. 해당 field들은 PCD 데이터베이스가 일반적으로는 비어있는 PcdNameTable 부분을 채우고 있기 때문에 존재한다.



## DumpDynPcd

MdeModulePkg에는 런타임에 Dynamic PCD 정보를 dump하는 application이 포함되어 있다.\
...
| Table         | \[PcdsFixedAtBuild]                                                          | \[PcdsFeatureFlag]                                           | \[PcdsPatchableInModule]                                                      | \[PcdsDynamic]                                                      | \[PcdsDynamicEx]                                                            |
| ------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------- |
...
| Scope:        | Every module have its own copy                                               | Every module have its own copy                               | Every module have its own copy                                                | <p>Global for<br>platform</p>                                       | <p>Global for<br>platform</p>                                               |
